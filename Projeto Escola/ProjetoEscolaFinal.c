...
#define Max_Alunos 100      // Aumentado para permitir mais alunos
#define Max_Professores 100 // Aumentado para permitir mais professores
#define Max_Disciplinas 100 // Aumentado para permitir mais disciplinas
...
typedef struct {
  ...
  char sexo;
  char nome[50];
  char cpf[15]; // Aumentado para 15 para incluir o '\0'
  data aniversario;
  int disciplinas;
} pessoas;
...
typedef struct {
  char nome[50];
  char codigo[9]; // Aumentado para 9 para incluir o '\0'
  char professor[50];
  float semestre;
...
void cadastrarDisciplina(materias *disciplinas, int max_Disciplinas,
                         pessoas *professores, int max_Professores);
...
int menu(char tipoMenu[20]) {
  ...
  if (strcmp(tipoMenu, "principal") == 0) { // Menu principal
    ...
    printf("\n");
  } else if (strcmp(tipoMenu, "cadastro") ==
             0) { // Menu alunos, professor e disciplina
    printf("0 - Voltar\n");
    printf("1 - Cadastrar\n");
    printf("2 - Atualizar\n");
    printf("3 - Excluir\n");
...
void printCadastroPessoa(pessoas *pessoa, int index) {
  ...
  printf("CPF: %s\n", pessoa[index].cpf);
  printf("Sexo: %c\n", pessoa[index].sexo);
  printf("Data de nascimento: %02d/%02d/%04d \n", pessoa[index].aniversario.dia,
         pessoa[index].aniversario.mes, pessoa[index].aniversario.ano);
  printf("Disciplinas: %d\n", pessoa[index].disciplinas); // DEBUG
  separadorTexto();
  printf("\n");
}
...
void printCadastroDisciplina(materias *disciplina, int index, bool comAluno) {
  ...
  printf("Total de vagas: %d / %d\n", disciplina[index].numAlunos,
         disciplina[index].numVagas);
...
pessoas getAluno(pessoas alunos[], int index) { return alunos[index]; }
...
void cadastrarPessoa(pessoas *cadastro, int maxPessoas, char tipoPessoa) {
  ...
  for (i = 0; i < maxPessoas; i++) {
    if (cadastro[i].matricula[0] ==
        '\0') { // Verifica se o campo matrícula está vazio
      ...
      do {
        printf("Informe a data de nascimento separada por um espaço (dd mm "
               "aaaa):\n");
        if (scanf("%d %d %d", &novoCadastro.aniversario.dia,
                  &novoCadastro.aniversario.mes,
                  &novoCadastro.aniversario.ano) != 3) {
...
int validarSexo(char sexo) {
  ...
  if (sexo != 'M' && sexo != 'F') {
    printf("Sexo inválido. Use 'M' para sexo masculino ou 'F' para o sexo "
           "feminino.\n");
    return 0;
  }
  return 1;
...
int validarData(data aniversario) {
  ...
  if (aniversario.dia < 1 || aniversario.dia > 31 || aniversario.mes < 1 ||
      aniversario.mes > 12 || aniversario.ano < 1900 ||
...
void cadastrarDisciplina(materias *disciplinas, int max_Disciplinas,
  ...
  for (i = 0; i < max_Disciplinas; i++) {
    if (disciplinas[i].codigo[0] ==
        '\0') { // Verifica se a disciplina não foi cadastrada
      ...
      do {
        ...
        if (strlen(novaDisciplina.codigo) > 8 ||
            strlen(novaDisciplina.codigo) == 0) {
          printf("O código deve ter entre 1 e 8 caracteres.\n");
        } else {
          toUpperString(
              novaDisciplina.codigo); // Converte o código para maiúsculas
        }
      } while (strlen(novaDisciplina.codigo) > 8 ||
               strlen(novaDisciplina.codigo) == 0);
      ...
      do {
        ...
        if (sscanf(input, "%f", &novaDisciplina.semestre) != 1 ||
            novaDisciplina.semestre <= 0) {
      ...
      do {
        ...
        if (scanf("%d", &novaDisciplina.numVagas) != 1 ||
            novaDisciplina.numVagas <= 0) {
      ...
      if (opcao == 's' || opcao == 'S') {
        ...
        do {
          ...
          for (int j = 0; j < max_Professores; j++) {
            if (strncmp(professores[j].matricula, prefixo_Professor,
                        strlen(prefixo_Professor)) == 0) {
...
void adicionarAlunoDisciplina(materias *disciplinas, int maxDisciplinas,
                              pessoas *alunos, int maxAlunos,
                              char matriculaAluno[]) {
  ...
  for (int i = 0; i < maxDisciplinas; i++) {
    if (disciplinas[i].codigo[0] !=
        '\0') { // Exibir apenas disciplinas cadastradas
  ...
  if (disciplinas[disciplinaEncontrada].temProfessor == 0) {
    printf("Disciplina sem professor vinculado. Para realizar matrícula nessa "
           "disciplina será necessário atualizar a disciplina e vincular um "
           "professor.\n");
    return;
  }
  ...
  for (int i = 0; i < disciplinas[disciplinaEncontrada].numAlunos; i++) {
    if (strcmp(getAluno(disciplinas[disciplinaEncontrada].aluno, i).matricula,
               matriculaAluno) == 0) {
  ...
  if (disciplinas[disciplinaEncontrada].numAlunos >=
  ...
  char confirmacao;
  printf("Você está prestes a adicionar o aluno %s à disciplina %s. Deseja "
         "continuar? (s/n): ",
         getAluno(alunos, alunoEncontrado).nome,
         disciplinas[disciplinaEncontrada].nome);
  scanf(" %c", &confirmacao); // Espaço antes de %c para consumir qualquer
                              // newline remanescente

  if (confirmacao == 's' || confirmacao == 'S') {
    // Adicionar aluno na disciplina
    setAluno(
        disciplinas[disciplinaEncontrada].aluno,
        disciplinas[disciplinaEncontrada].numAlunos,
        getAluno(alunos,
                 alunoEncontrado)); // Usando o setter para adicionar o aluno
    disciplinas[disciplinaEncontrada]
        .numAlunos++; // Atualiza o número de alunos
...
void desmatricularAlunoDisciplina(materias disciplinas[], pessoas *alunos,
                                  int numDisciplinas) {
  ...
  printf(
      "Informe a matrícula do aluno a ser desmatriculado da disciplina %s:\n",
  ...
  // Buscar o aluno pela matrícula
  for (int i = 0;
       i < getDisciplina(disciplinas, disciplinaEncontrada).numAlunos; i++) {
    if (strcmp(getAluno(disciplinas[disciplinaEncontrada].aluno, i).matricula,
               matriculaAluno) == 0) {
      alunoEncontrado = i;
      break;
  ...
  char confirmacao;
  printf(
      "Você tem certeza que deseja desmatricular o aluno %s da disciplina %s? "
      "(S/N): ",
  ...
  if (confirmacao == 'S' || confirmacao == 's') {
    ...
    for (int i = alunoEncontrado;
         i < getDisciplina(disciplinas, disciplinaEncontrada).numAlunos - 1;
         i++) {
      setAluno(disciplinas[disciplinaEncontrada].aluno, i,
               getAluno(disciplinas[disciplinaEncontrada].aluno,
                        i + 1)); // Usar o setter para mover alunos
    }
    disciplinas[disciplinaEncontrada].numAlunos--;
...
void atualizarDisciplina(materias *disciplinas, int max_Disciplinas,
                         pessoas *professores, int max_Professores) {
  char codigoBusca[9];
  int encontrou = -1;
  ...
  toUpperString(
      codigoBusca); // Aceitar código tanto em maiúsculas quanto minúsculas
  ...
  printf("Informe o novo nome da disciplina (ou pressione ENTER para manter o "
         "nome atual: %s):\n",
         disciplinas[encontrou].nome);
  char novoNome[50];
  ...
  printf("Informe a matrícula do novo professor (ou pressione ENTER para "
         "manter o professor atual: %s):\n",
  ...
  if (strlen(novaMatriculaProf) > 0) {
    int professorEncontrado =
        0; // Variável para verificar se o professor foi encontrado

    // Busca o professor pela matrícula fornecida
    for (int j = 0; j < max_Professores; j++) {
      if (strcmp(professores[j].matricula, novaMatriculaProf) == 0) {
        strcpy(disciplinas[encontrou].professor,
               professores[j].nome); // Vincula o nome do professor à disciplina
        disciplinas[encontrou].temProfessor =
            1; // Atualiza para indicar que a disciplina agora tem um professor
               // vinculado
    ...
    if (!professorEncontrado) {
      printf("Professor com matrícula %s não encontrado. O professor não foi "
             "vinculado.\n",
             novaMatriculaProf);
  ...
  float novoSemestre;
  printf("Informe o novo semestre (ou pressione ENTER para manter o semestre "
         "atual: %.1f):\n",
         disciplinas[encontrou].semestre);
  fgets(codigoBusca, sizeof(codigoBusca),
        stdin); // Usar o mesmo buffer para leitura
  codigoBusca[strcspn(codigoBusca, "\n")] = '\0'; // Remove o newline
  ...
  int novasVagas;
  printf("Informe o novo número de vagas (ou pressione ENTER para manter o "
         "número atual: %d):\n",
...
void atualizarPessoa(pessoas *cadastro, int maxPessoas, int tipoPessoa) {
  ...
  for (int i = 0; i < maxPessoas; i++) {
    ...
    if ((tipoPessoa == 1 &&
         strncmp(getAluno(cadastro, i).matricula, prefixo_Aluno,
                 strlen(prefixo_Aluno)) == 0) ||
        (tipoPessoa == 2 &&
         strncmp(getProfessor(cadastro, i).matricula, prefixo_Professor,
                 strlen(prefixo_Professor)) == 0)) {
      if (strcmp(getAluno(cadastro, i).matricula, matricula) == 0 ||
          strcmp(getProfessor(cadastro, i).matricula, matricula) == 0) {
        char novoNome[50]; // Tamanho do nome conforme definido na struct
        char novoCPF[15];  // Tamanho do CPF conforme definido na struct
        char
            novoSexo[2]; // Buffer para o sexo, permitindo também a string vazia
        pessoas pessoaAtual = (tipoPessoa == 1) ? getAluno(cadastro, i)
                                                : getProfessor(cadastro, i);
        ...
        printf("Sexo: %c\n", pessoaAtual.sexo);
        printf("Data de nascimento: %02d/%02d/%04d\n",
               pessoaAtual.aniversario.dia, pessoaAtual.aniversario.mes,
               pessoaAtual.aniversario.ano);
        ...
        printf("Informe a nova data de nascimento separada por espaço: dd mm "
               "aaaa (ou pressione enter para manter):\n");
        char dataNascimento[20];
        getchar();
        fgets(dataNascimento, sizeof(dataNascimento), stdin);
        if (sscanf(dataNascimento, "%d %d %d", &cadastro[i].aniversario.dia,
                   &cadastro[i].aniversario.mes,
                   &cadastro[i].aniversario.ano) == 3) {
          printf("Data de nascimento atualizada para: %02d/%02d/%04d.\n",
                 cadastro[i].aniversario.dia, cadastro[i].aniversario.mes,
                 cadastro[i].aniversario.ano);
        } else {
...
void excluirPessoa(pessoas *cadastro, int *maxPessoas, int tipoPessoa) {
  ...
  for (int i = 0; i < *maxPessoas; i++) {
    // Verifica se a matrícula corresponde à que está cadastrada
    if ((tipoPessoa == 1 && strncmp(cadastro[i].matricula, prefixo_Aluno,
                                    strlen(prefixo_Aluno)) == 0) ||
        (tipoPessoa == 2 && strncmp(cadastro[i].matricula, prefixo_Professor,
                                    strlen(prefixo_Professor)) == 0)) {
      if (strcmp(cadastro[i].matricula, matricula) == 0) {
        // Encontrou a matrícula, solicita confirmação
        char confirmacao;
        printf("Você tem certeza que deseja excluir a pessoa %s? (s/n): ",
               cadastro[i].nome);
        ...
        if (confirmacao == 's' || confirmacao == 'S') {
          ...
          for (int j = i; j < *maxPessoas - 1; j++) {
            cadastro[j] =
                cadastro[j + 1]; // Move os registros para preencher a lacuna
          }
  ...
  for (int i = 0; i < max_Disciplinas; i++) {
    if (strcmp(disciplinas[i].codigo, codigo) == 0) {
      ...
      if (disciplinas[i].numAlunos > 0) {
        printf("Não é possível excluir a disciplina, pois existem %d aluno(s) "
               "matriculado(s).\n",
               disciplinas[i].numAlunos);
      } else {
        // Solicita confirmação antes de excluir
        char confirmacao;
        printf("Você tem certeza que deseja excluir a disciplina %s (Cód.: "
               "%s)? (S/N)\n",
        ...
        if (confirmacao == 'S' || confirmacao == 's') {
          ...
          disciplinas[i].codigo[0] =
              '\0'; // Marca a disciplina como não cadastrada
...
void listarPessoa(pessoas *pessoa, int Max_Pessoas, int tipoPessoa,
                  char ordenacao[20]) {
  // relatórios de listagem de pessoas (alunos ou professores) - tipoPessoa:
  // (1-aluno,2-professor); ordenacao:
  // ("nenhuma","sexo","alfabetica","nascimento","matriculaMenos3")
  ...
  if (strcmp(ordenacao, "nenhuma") == 0) {
    ...
    for (int i = 0; i < Max_Pessoas; i++) {
      if (strcmp(pessoa[i].matricula, "") != 0) {
        ...
        // ocupada (matrícula não foi excluída)
    ...
    char sexo;
    printf("Informe o sexo que deseja listar (M para masculino e F para "
           "feminino): ");
    scanf(" %c", &sexo);
    ...
    // system("clear");
      ...
      printf("\n");
      for (int i = 0; i < Max_Pessoas; i++) {
        if ((pessoa[i].sexo == sexo || pessoa[i].sexo == sexo - 32) &&
            strcmp(pessoa[i].matricula, "") != 0) {
          // verifica se a matrícula é diferente de "" e se o sexo é igual ao
          // informado
          printCadastroPessoa(pessoa, i);
          contador++;
        }
    ...
    //É necessário garantir que quando for feito o cadastro de um novo nome,
    // todas as letras estejam maiúculas ou minúsculas
    pessoas alfabetica[Max_Pessoas];
    for (int i = 0; i < Max_Pessoas; i++) {
      alfabetica[i] = pessoa[i];
    }
    for (int i = 1; i < Max_Pessoas;
         i++) // insertion sort para ordenar os nomes alfabeticamente
    {
      int j = i;
    ...
    for (int i = 0; i < Max_Pessoas;
         i++) { // cópia da array para a array auxiliar de nascimento
      nascimento[i] = pessoa[i];
    }
    for (int i = 1; i < Max_Pessoas;
         i++) { // insertion sort para ordenar por datas por ano de nascimento
      pessoas aux = nascimento[i];
      int j = i - 1;

      while (j >= 0 && nascimento[j].aniversario.ano > aux.aniversario.ano ||
             (nascimento[j].aniversario.ano == aux.aniversario.ano &&
              nascimento[j].aniversario.mes > aux.aniversario.mes) ||
             (nascimento[j].aniversario.ano == aux.aniversario.ano &&
              nascimento[j].aniversario.mes == aux.aniversario.mes &&
              nascimento[j].aniversario.dia > aux.aniversario.dia)) {
        // Para inverter ordem dos anos, alterar primeira comparação para
        // nascimento.ano < aux.aniversario.ano
    ...
    printf("Lista de %s cadastrados em ordem de nascimento:\n\n",
           (tipoPessoa == 1) ? "Alunos" : "Professores");
    for (int i = 0; i < Max_Pessoas; i++) {
      if (strcmp(nascimento[i].matricula, "") != 0) {
        // se a matrícula for diferente de "", significa que a posição está
        // ocupada (matrícula não foi excluída)
    ...
    // se a ordenação foi especificada para alunos matriculados em menos de 3
    // disciplinas
...
void listarTodasPessoas(pessoas *pessoas1, int Max_Pessoas1, pessoas *pessoas2,
                        int Max_Pessoas2, char ordenacao[20]) {
  ...
  if (strcmp(ordenacao, "nenhuma") == 0) {
    ...
    for (int j = 0; j < Max_Todas_Pessoas; j++) {
      if (strcmp(todos[j].matricula, "") != 0) {
        // se a matrícula for diferente de "", significa que a posição está
        // ocupada e a matrícula não foi excluída
        printCadastroPessoa(todos, j);
        contador++;
      }
    }
  } else if (strcmp(ordenacao, "aniversariantes") == 0) {
    // se a ordenação foi especificada como aniversariantes, listar os
    ...
    for (int j = 0; j < Max_Todas_Pessoas; j++) {
      if (strcmp(todos[j].matricula, "") != 0 &&
          todos[j].aniversario.mes == mes_atual) {
        // se a matrícula for diferente de "", significa que a posição está
        // ocupada e a matrícula não foi excluída verifica se o mês do
        // aniversário é o mês atual
        printCadastroPessoa(todos, j);
        contador++;
      }
    }
  } else if (strcmp(ordenacao, "letras") == 0) {
    // se a ordenação foi especificada como letras, listar os pessoas que contem
    // as letras
    char letras[50];
    ...
    if (strlen(letras) < 3) {
      ...
      for (int j = 0; j < Max_Todas_Pessoas; j++) {
        if (strcmp(todos[j].matricula, "") != 0) {
          if (strstr(todos[j].nome, letras) !=
              NULL) { // Verifica se o nome contém a sequência de letras
            printCadastroPessoa(todos, j);
            contador++;
          }
        }
...
void listarDisciplinas(materias *disciplina, int Max_disciplinas,
                       char ordenacao[20]) {
  // relatórios de listagem de disciplinas (cadastradas,dados de uma e
  // disciplinas com mais de 40 vagas) ordenacao: ("nenhuma","mais40","uma")
  ...
  if (strcmp(ordenacao, "nenhuma") == 0) {
    ...
    fgets(codigo, sizeof(codigo), stdin);
    toUpperString(codigo);
    for (int i = 0; i < strlen(codigo);
         i++) { // Garante que todas as letras armazenadas sejam maíusculas
    ...
    for (int i = 0; i < Max_Disciplinas; i++) {
      if (strcmp(disciplina[i].codigo, "") != 0 &&
          strcmp(disciplina[i].codigo, codigo) == 0) {
        // se o código for diferente de "", significa que a posição está ocupada
        //(disciplina não foi excluída)
...
// void inicializarDisciplina  (materias *disciplinas, int Max_Disciplinas) {
//       for (int i = 0; i < Max_Disciplinas; i++) {
//           disciplinas[i].codigo[0] = '\0';
//           disciplinas[i].nome[0] = '\0';
//           disciplinas[i].professor[0] = '\0';
//           disciplinas[i].semestre = 0.0;
//           disciplinas[i].numAlunos = 0;
//           disciplinas[i].numVagas = 0;
//       }
// }

// // Inicialização para evitar lixo na memória (aluno e professor)

void inicializarPessoa(pessoas *pessoas, int Max_Pessoas) {
  for (int i = 0; i < Max_Pessoas; i++) {
    pessoas[i].matricula[0] = '\0';
    pessoas[i].nome[0] = '\0';
    pessoas[i].aniversario.dia = 0;
    pessoas[i].aniversario.mes = 0;
    pessoas[i].aniversario.ano = 0;
    pessoas[i].sexo = '\0';
    pessoas[i].disciplinas = 0;
  }
}
...
int main(void) {
  ...
  inicializarPessoa(aluno, Max_Alunos);
  inicializarPessoa(professor, Max_Professores);

  bool encerrar = false;
  bool voltar = false;
  int maxAlunos = Max_Alunos;
  int maxProfessores = Max_Professores;
  int maxDisciplinas = Max_Disciplinas;
    ...
    case 3: { // Menu Disciplina
      ...
      while (!voltar) {
        ...
        case 1: { // Cadastro de nova disciplina
          ...
          cadastrarDisciplina(disciplinas, Max_Disciplinas, professor,
                              Max_Professores);
          break;
        }
        case 2: { // Atualizar dados de disciplina
          atualizarDisciplina(disciplinas, Max_Disciplinas, professor,
                              Max_Professores);
          break;
        }
      ...
      while (!voltar) {
        ...
        case 1: { // Matricular aluno em uma disciplina
          ...
          if (disciplinas[0].numAlunos < Max_Alunos_Disciplinas) {
            adicionarAlunoDisciplina(&disciplinas[0], Max_Disciplinas, aluno,
                                     Max_Alunos, matriculaAluno);
            espera();
          } else {
    ...
    case 5: { // Menu Relatórios
      ...
      while (!voltar) {
        ...
        case 1: { // listagem de alunos sem ordenação do vetor
          // inserir função para listagem de pessoas(alunos) sem ordenação do
          // vetor
          listarPessoa(aluno, maxAlunos, 1, "nenhuma");
          espera();
          break;
        }
        case 2: { // listagem de alunos por sexo
          // inserir função para listagem de pessoas(alunos) por sexo informado
          listarPessoa(aluno, maxAlunos, 1, "sexo");
          espera();
          break;
        }
        case 3: { // listagem de alunos por ordem alfabética
          // inserir função para listagem de pessoas(alunos) com ordenação do
          // vetor em ordem alfabética
          listarPessoa(aluno, maxAlunos, 1, "alfabetica");
          espera();
          break;
        }
        case 4: { // listagem de alunos por ordem de nascimento
          // inserir função para listagem de pessoas(alunos) com ordenação do
          // vetor em ordem de nascimento
          listarPessoa(aluno, maxAlunos, 1, "nascimento");
          espera();
          break;
        }
        case 5: { // listagem de alunos matriculados em menos de 3 disciplinas
          // inserir função para listagem de alunos matriculados em menos de 3
          // disciplinas
          listarPessoa(aluno, maxAlunos, 1, "matriculaMenos3");
          espera();
          break;
        }
        case 6: { // listagem de professor sem ordenação do vetor
          // inserir função para listagem de pessoas(professor) sem ordenação do
          // vetor
          listarPessoa(professor, maxProfessores, 2, "nenhuma");
          espera();
          break;
        }
        case 7: { // listagem de professor por sexo
          // inserir função para listagem de pessoas(professor) por sexo
          // informado
          listarPessoa(professor, maxProfessores, 2, "sexo");
          espera();
          break;
        }
        case 8: { // listagem de professor por ordem alfabética
          // inserir função para listagem de pessoas(professor) com ordenação do
          // vetor em ordem alfabética
          listarPessoa(professor, maxProfessores, 2, "alfabetica");
          espera();
          break;
        }
        case 9: { // listagem de professor por ordem de nascimento
          // inserir função para listagem de pessoas(professor) com ordenação do
          // vetor em ordem de nascimento
          listarPessoa(professor, maxProfessores, 2, "nascimento");
          espera();
          break;
        }
        case 10: { // listagem de disciplinas cadastradas
          // inserir função para listagem de todas as disciplinas cadastradas
          // (nomes das disciplinas)
          listarDisciplinas(disciplinas, maxDisciplinas, "nenhuma");
          espera();
          break;
        }
        case 11: { // listagem de disciplinas com mais de 40 vagas
          // inserir função para listagem de disciplinas com mais de 40 vagas e
          // pofessor correspondente (nome da disciplina e nome do professor)
          listarDisciplinas(disciplinas, maxDisciplinas, "mais40");
          espera();
          break;
        }
        case 12: { // listagem dos dados de uma disciplina
          // inserir função para listagem de todos os dados de uma disciplina
          // selecionada pelo usuário
          listarDisciplinas(disciplinas, maxDisciplinas, "uma");
          espera();
          break;
        }
        case 13: { // listagem dos aniversariantes do mês
          // inserir função para listagem de todos as pessoas que fazem
          // aniversário no mês corrente e suas respectivas datas de aniversário
          // (dia/mês)
          listarTodasPessoas(aluno, maxAlunos, professor, maxProfessores,
                             "aniversariantes");
          espera();
          break;
        }
        case 14: { // listagem de todas as pessoas cadastradas
          // inserir função para listagem de pessoas(todas) com base nas letras
          // informadas
          listarTodasPessoas(aluno, maxAlunos, professor, maxProfessores,
                             "letras");
          espera();
